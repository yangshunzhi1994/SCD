...
class VOCDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {'feature': torch.from_numpy(img), 'label': target, 'index': index}
        return sample
...
def detection_collate(batch):
    ...
    imgs = []
    index = []
    for _, batch_sample in enumerate(batch):
        sample = []
        sample.append(batch_sample['feature'])
        sample.append(batch_sample['label'])
        sample.append(batch_sample['index'])
        ...
        for _, tup in enumerate(sample):
            if torch.is_tensor(tup):
                ...
                index.append(tup)
    return (torch.stack(imgs, 0), targets, torch.tensor(index, dtype=torch.int64))
