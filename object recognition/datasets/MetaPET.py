...
class MetaPET(data.Dataset):
    def __init__(self, split='train', transform=None, student_norm=None, teacher_norm=None):
        ...
        self.split = split
        self.data = h5py.File('datasets/PET_MetaData_100.h5', 'r', driver='core')
        ...
        if self.split == 'train':
            ...
            self.train_data = self.train_data.reshape((2724, 100, 100, 3))
            ...
            self.valid_data = self.valid_data.reshape((956, 100, 100, 3))
            ...
            self.test_data = self.test_data.reshape((3669, 100, 100, 3))
...
class PET_teacher(data.Dataset):
    def __init__(self, split='Training', transform=None):
        self.transform = transform
        self.split = split 
        self.data = h5py.File('datasets/PET_data_100.h5', 'r', driver='core')
        if self.split == 'Training':
            self.train_data = self.data['train_data_pixel']
            self.train_labels = self.data['train_data_label']
            self.train_data = np.asarray(self.train_data)
            self.train_data = self.train_data.reshape((3680, 100, 100, 3))

        else:
            self.PrivateTest_data = self.data['valid_data_pixel']
            self.PrivateTest_labels = self.data['valid_data_label']
            self.PrivateTest_data = np.asarray(self.PrivateTest_data)
            self.PrivateTest_data = self.PrivateTest_data.reshape((3669, 100, 100, 3))
